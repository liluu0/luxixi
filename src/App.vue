...
const openBrain=()=>{history.pushState({},'', '/works/brain-games');path.value=location.pathname;window.scrollTo(0,0)}
...
<template><CityHeatmap v-if="path==='/works/city-heatmap'" :on-back="closeWork"/><CastleBattle v-else-if="path==='/works/castle-battle'" :on-back="closeWork"/><BrainGames v-else-if="path==='/works/brain-games'" :on-back="closeWork"/><HomePage v-else :on-open="openWork" :on-open-castle="openCastle" :on-open-brain="openBrain"/></template>
